...
- Nen tranh ten folder co dau cach cho noi dung publish. Dung cac folder topic nhu `entity-relationship-model`, `relational-model-and-functional-dependencies`, `normalization` thay vi `Data Modeling`.
- Neu mot bai co `permalink`, nen dat permalink trung voi folder bai hoc, vi du `permalink: /entity-relationship-model/er-model/`. Khi do link anh local dang `images/file.png` se dung ca tren local va GitHub Pages.
