...
A. Thay thế hoàn toàn SQL  
B. Biểu diễn các yêu cầu dữ liệu phức tạp hơn ER Model cơ bản  
C. Tối ưu tốc độ truy vấn vật lý  
D. Sao lưu dữ liệu tự động  

**Câu 2.** Khái niệm nào sau đây thuộc EER Model?

A. Superclass và subclass  
B. File system  
C. Packet switching  
...
B. Vì khó thể hiện trực tiếp phân cấp, kế thừa và phân loại thực thể phức tạp  
C. Vì không dùng được khóa chính  
D. Vì không hỗ trợ thuộc tính  
...
A. Entity set tổng quát chứa các thuộc tính chung  
B. Một bảng chỉ chứa khóa ngoại  
...
B. Kế thừa thuộc tính và quan hệ từ superclass  
C. Chỉ dùng trong mô hình vật lý  
...
A. Để mô hình phản ánh đúng quy tắc nghiệp vụ  
...
A. Một subclass được hình thành từ nhiều superclass khác nhau  
B. Một thuộc tính không có kiểu dữ liệu  
...
A. Khi cần xem một relationship như một đối tượng cấp cao hơn  
B. Khi không có entity nào  
C. Khi chỉ có một thuộc tính  
...
A. Total/partial và disjoint/overlapping  
B. Màu sắc của sơ đồ  
...
A. Để tránh trùng lặp dữ liệu trong mô hình  
B. Để bỏ khóa chính  
C. Để không cần bảng  
D. Để xóa subclass  
...
A. Hệ thống có nhiều loại thực thể phân cấp  
B. Hệ thống chỉ có một biến số  
C. Hệ thống không có dữ liệu  
D. Hệ thống không cần mô hình hóa  
...
A. Specialization trong mô hình dữ liệu  
B. Sao lưu dữ liệu  
C. Xóa bảng  
D. Tạo trigger  
...
A. Làm rõ các nhóm đối tượng và quy tắc phân loại  
B. Tự động viết toàn bộ ứng dụng  
C. Thay thế người dùng cuối  
D. Xóa dữ liệu cũ  

**Câu 2.** Trong thiết kế database, EER giúp gì?

A. Chọn chiến lược mapping superclass/subclass  
B. Tăng tốc CPU  
C. Tạo giao diện web  
...
A. Mô hình ER mở rộng để biểu diễn yêu cầu dữ liệu phức tạp hơn  
...
A. Chứa các thuộc tính chung của nhiều subclass  
B. Chỉ chứa dữ liệu tạm thời  
C. Chỉ dùng để biểu diễn khóa ngoại  
...
A. Chia entity tổng quát thành các entity chuyên biệt  
B. Gộp nhiều database thành một database  
C. Sao lưu dữ liệu  
...
A. Mọi entity trong superclass phải thuộc ít nhất một subclass  
B. Không entity nào được thuộc subclass  
C. Một entity chỉ có đúng một thuộc tính  
...
A. Một số entity trong superclass có thể không thuộc subclass nào  
B. Mọi entity bắt buộc thuộc mọi subclass  
C. Không tồn tại superclass  
D. Subclass không được kế thừa thuộc tính  
...
A. `LibraryMember` là union của `Student`, `Faculty`, `Staff`  
B. `Employee` có thuộc tính `salary`  
...
A. Tạo một bảng cho superclass và mỗi subclass  
B. Chỉ dùng file văn bản  
C. Không cần bảng  
