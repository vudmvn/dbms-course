...
\vspace{2mm}
\begin{alertblock}{Lưu ý về hình minh họa}
Nguồn tham khảo gốc không cung cấp hình minh họa riêng cho Simple Attribute. Trong ER Diagram, simple attribute vẫn được biểu diễn bằng oval đơn, tương tự các attribute thông thường như \texttt{Name}, \texttt{DOB} hoặc \texttt{Age}.
\end{alertblock}
\end{frame}
...
\centering
\imageplaceholder{simple-attribute.png}{Composite Attribute trong ER Model}
\end{frame}
...
\centering
\imageplaceholder{composite-attribute.png}{Multivalued Attribute trong ER Model}
\end{frame}
...
\centering
\imageplaceholder{multivalued-attribute.png}{Derived Attribute trong ER Model}
\end{frame}

%------------------------------------------------
\begin{frame}{Entity và các Attribute tổng hợp}
\small
Khi kết hợp các loại attribute trong một ER Diagram, entity \texttt{Student} có thể có:
\begin{itemize}
\smallitem
  \item \texttt{Roll\_No} là key attribute.
  \item \texttt{Name} và \texttt{DOB} là simple attribute.
  \item \texttt{Phone\_No} là multivalued attribute.
  \item \texttt{Age} là derived attribute.
  \item \texttt{Address} là composite attribute gồm \texttt{Street}, \texttt{City}, \texttt{State} và \texttt{Country}.
\end{itemize}
\end{frame}

%------------------------------------------------
\begin{frame}{Hình minh họa: Entity và các Attribute tổng hợp}
\centering
\imageplaceholder{derived-attribute.png}{Entity Student và các loại Attribute trong ER Model}
\end{frame}
...
%------------------------------------------------
\begin{frame}{Hình minh họa: Relationship Type}
\centering
\imageplaceholder{relationship-set.png}{Relationship Type giữa Student và Course}

\vspace{2mm}
{\small Hình 5a. Relationship type giữa Student và Course.}
\end{frame}

%------------------------------------------------
\begin{frame}{Hình minh họa: Relationship Set}
\centering
\imageplaceholder{recursive-relationship.png}{Relationship set của loại Enrolled in}

\vspace{2mm}
{\small Hình 5b. Relationship set của loại Enrolled in.}
\end{frame}
...
\centering
\imageplaceholder{binary-relationship.png}{Unary hoặc Recursive Relationship}
\end{frame}
