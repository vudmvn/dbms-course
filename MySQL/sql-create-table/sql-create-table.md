...
B. Tạo một bảng mới trong cơ sở dữ liệu  
...
D. Số lượng bản ghi ban đầu  

**Câu 3.** Sau khi thực thi thành công `CREATE TABLE`, kết quả là gì?

A. Một bảng mới được tạo trong cơ sở dữ liệu  
B. Tất cả bảng bị xóa  
...
A. Đặt tên bảng  
B. Kiểm tra điều kiện hợp lệ của dữ liệu  
C. Tự động sao lưu bảng  
...
A. Vì số điện thoại có thể có số 0 đầu hoặc ký tự `+`  
B. Vì số điện thoại luôn dùng để tính toán  
C. Vì `INT` lưu được mọi ký tự  
D. Vì `VARCHAR` chỉ lưu số nguyên  

**Câu 3.** `Age INT` nghĩa là gì?

A. Tuổi được lưu dưới dạng số nguyên  
B. Tuổi được lưu dưới dạng ngày tháng  
C. Tuổi là khóa chính  
D. Tuổi là dữ liệu nhị phân  
...
A. Gán giá trị mặc định cho cột  
B. Xóa bảng mặc định  
C. Đổi tên cột  
D. Truy vấn dữ liệu  
...
A. Rõ nghĩa và nhất quán  
B. Càng ngắn càng tốt dù khó hiểu  
...
A. Khi muốn tạo bảng mới từ kết quả truy vấn  
B. Khi muốn xóa toàn bộ database  
C. Khi muốn đổi mật khẩu hệ điều hành  
...
A. Lưu kết quả truy vấn vào bảng mới  
B. Đổi tên máy tính  
C. Xóa trình duyệt  
D. Tạo file PDF  
...
B. `CustomerID` là khóa chính dạng số nguyên  
C. `CustomerID` có thể trùng lặp tùy ý  
D. `CustomerID` chỉ lưu ngày tháng  
...
A. Một số ràng buộc, khóa và chỉ mục  
B. Dữ liệu được truy vấn  
C. Tên cột được chọn  
...
B. `CREATE TABLE ALWAYS`  
C. `CREATE TABLE REPEAT`  
...
A. Xem cấu trúc bảng `Customer`  
B. Xóa bảng `Customer`  
C. Thêm dữ liệu vào bảng `Customer`  
D. Đổi tên bảng `Customer`  
